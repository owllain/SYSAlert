    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-white dark:bg-[#0f1117]">
        <div className="flex flex-col items-center gap-4">
          <div className="w-12 h-12 rounded-[10px] bg-[#181d26] dark:bg-[#2d3140] flex items-center justify-center animate-pulse">
            <span className="text-white text-lg font-medium">SA</span>
          </div>
          <p className="text-[#41454d] dark:text-[#9ea3b0] text-sm">Cargando sistema...</p>
        </div>
      </div>
    )
  }
    ...
    <TooltipProvider>
      <div className="min-h-screen flex flex-col bg-white dark:bg-[#0f1117]">
        ...
        <div className="flex flex-1">
          ...
          <div
            className={`fixed inset-0 bg-black/30 dark:bg-black/50 z-30 lg:hidden transition-opacity duration-300 ${sidebarOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
        ...
        {/* Footer */}
        <footer className="border-t border-[#dddddd] dark:border-[#2d3140] bg-[#f8fafc] dark:bg-[#1a1d27] py-3 sm:py-4 px-4 sm:px-6 mt-auto">
          <div className="flex items-center justify-between max-w-[1280px] mx-auto">
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="w-5 h-5 rounded-[4px] bg-[#181d26] dark:bg-[#2d3140] flex items-center justify-center">
                <span className="text-white text-[8px] font-medium">SA</span>
              </div>
              <span className="text-xs text-[#41454d] dark:text-[#9ea3b0]">
                <span className="hidden sm:inline">Sistema de Alertas Interbancario v2.0</span>
                <span className="sm:hidden">SAI v2.0</span>
              </span>
              <span className="text-[#dddddd] dark:text-[#2d3140] hidden sm:inline">|</span>
              <span className="text-xs text-[#41454d] dark:text-[#9ea3b0] hidden sm:inline">
                {currentUser?.financialEntityName || ''}
              </span>
            </div>
            <div className="flex items-center gap-2 sm:gap-3">
              <span className="text-xs text-[#9297a0] dark:text-[#6b7080]">
                © {new Date().getFullYear()}
              </span>
              <span className="text-[#dddddd] dark:text-[#2d3140] hidden sm:inline">|</span>
              <span className="text-xs text-[#9297a0] dark:text-[#6b7080] hidden sm:inline">
